...
import CommandFilters, {
  AlphabeticalOrder,
  RoleSort,
} from "@/features/commands/CommandFilters";
...
const Commands = memo(function Commands() {
  ...
  const [alphabeticalOrder, setAlphabeticalOrder] =
    useState<AlphabeticalOrder>("asc");
  ...
  const filteredCommands = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (cmd) =>
          ...
          cmd.commandGroups?.some((group) =>
            normalizeForSearch(group).includes(query),
          ),
      );
    }
    ...
    result.sort((a, b) => {
      if (roleSort !== "off") {
        const roleDiff =
          roleSort === "asc"
            ? PERMISSION_PRIORITY[a.permission] -
              PERMISSION_PRIORITY[b.permission]
            : PERMISSION_PRIORITY[b.permission] -
              PERMISSION_PRIORITY[a.permission];
  ...
  return (
    <PageWrapper>
      <main className="flex-1 mx-auto w-full max-w-[95vw] px-4 sm:px-6 lg:px-10 xl:px-16 py-12 md:py-20">
        ...
        <div
          ref={gridRef}
          className="grid grid-cols-1 md:grid-cols-2 3xl:grid-cols-3 4xl:grid-cols-4 5xl:grid-cols-5 6xl:grid-cols-6 gap-4 items-stretch"
        >
          {filteredCommands.length > 0 ? (
            filteredCommands.map((command, index) => {
              ...
              return (
                <div
                  key={command.id}
                  className={`${isFocused ? "md:col-span-2 3xl:col-span-3 4xl:col-span-4 5xl:col-span-5 6xl:col-span-6" : ""} h-full`}
                >
                  <CommandCard
                    command={command}
                    orderNumber={index + 1}
                    isFocused={isFocused}
                    onFocus={
                      canFocus ? () => handleFocus(command.id) : undefined
                    }
                  />
                </div>
              );
