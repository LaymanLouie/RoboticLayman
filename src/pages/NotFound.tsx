...
const NotFound = () => {
  ...
  const getPosition = useCallback(
    (configIndex: number, progress: number): ParticleState => {
      ...
      const outerX =
        centerX + Math.cos(config.angle) * centerX * config.distance;
      const outerY =
        centerY + Math.sin(config.angle) * centerY * config.distance;
      ...
      <div className="relative flex-1 flex items-center justify-center overflow-hidden min-h-[calc(100vh-200px)]">
        <div className="absolute inset-0 overflow-hidden">
          {configsRef.current.map((config, i) => {
            const pos =
              particlesRef.current[i] || getPosition(i, config.phaseOffset);
            return (
              <div
                key={i}
