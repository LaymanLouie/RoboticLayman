...
export function parseMSTTime(timeStr: string): Date | null {
  ...
  const now = new Date();
  return new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), hours + 7, 0));
}
...
export function formatMSTToLocalAbsolute(mstDateTimeStr: string, options?: DateTimeFormatOptions): string {
  const utcDate = parseMSTDateTime(mstDateTimeStr);
