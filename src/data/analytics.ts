...
const localModules = import.meta.glob<{ ANALYTICS_LOCAL: AnalyticsDay[] }>(
  "./analytics.local.ts",
  { eager: true },
);
const localAnalytics: AnalyticsDay[] | null = (() => {
  for (const mod of Object.values(localModules)) {
    if (
      mod &&
      Array.isArray(
        (mod as { ANALYTICS_LOCAL?: AnalyticsDay[] }).ANALYTICS_LOCAL,
      )
    ) {
...
export const HAS_LOCAL_ANALYTICS =
  localAnalytics !== null && localAnalytics.length > 0;
