...
function parseDate(date: Date | string, isProjectFormat: boolean): Date {
  if (date instanceof Date) {
    return date;
  }

  if (isProjectFormat) {
    return parseProjectDateTime(date);
  }
...
const DateTime = memo(function DateTime({
  ...
  const parsedDate = useMemo(
    () => parseDate(date, isProjectFormat),
    [date, isProjectFormat],
  );
  ...
  useEffect(() => {
    ...
    const usesRelative =
      format === "relative" || format === "smart" || format === "compact";
  ...
  const tooltipContent = useMemo(() => {
    ...
    if (isProjectFormat && typeof date === "string") {
  ...
  }, [showTooltip, isProjectFormat, date, parsedDate]);
  ...
  const content = (
    ...
    >
      ...
      {timezoneAbbr && (
        <span
          className={cn(
            "text-muted-foreground/60 text-sm ml-1",
            timezoneClassName,
          )}
        >
          {timezoneAbbr}
