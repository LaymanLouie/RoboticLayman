...
const FillableUsage = memo(function FillableUsage({
  prefix,
  usage,
  className,
  onChange,
  ...
  const placeholders = useMemo(
    () =>
      Array.from(
        new Set(
          tokens.filter((t) => t.type === "placeholder").map((t) => t.value),
        ),
      ),
    [tokens],
  );
  ...
  if (!usage || placeholders.length === 0) {
    return usage ? (
      <code
        className={cn("text-muted-foreground text-base font-mono", className)}
      >
        {usage}
      </code>
    ) : null;
  ...
  return (
    <span
      className={cn(
        "inline-flex flex-wrap items-center gap-1.5 text-base font-mono",
        className,
      )}
    >
      {tokens.map((token, i) => {
          ...
          <span key={i} className="inline-flex items-center">
            <span
              className={cn(
                ...
                filled && "text-muted-foreground/30",
              )}
            >
              [
            </span>
            <input
              ...
              maxLength={20}
              style={{
                width: `${Math.max(token.value.length, (values[token.value] ?? "").length || 1) + 0.5}ch`,
              }}
              className={cn(
...
const CommandCard = memo(function CommandCard({
  ...
  const hasParameterGroups =
    command.parameterGroups && command.parameterGroups.length > 0;
  const hasVariations =
    command.usageVariations && command.usageVariations.length > 0;
          ...
          <div className="flex-1 min-w-0">
            ...
            {(command.massCompatible ||
              (command.commandGroups &&
                command.commandGroups.filter((g) => g).length > 0)) && (
...
function VariantUsageRow({ name, usage }: { name: string; usage?: string }) {
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      {usage && (
        <FillableUsage prefix={name} usage={usage} onChange={setResolved} />
      )}
      <CopyButton text={resolved} />
    </div>
  );
