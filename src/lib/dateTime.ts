...
function projectWallClockToUTC(
  year: number,
  month: number,
  day: number,
  hour: number,
  ...
  const offsetMinutes = getZoneOffsetMinutes(
    DateTimeConfig.storageTimezone,
    tentative,
  );
...
export function getUserTimezoneName(): string {
  ...
  const match = timeZoneStr.match(
    /[A-Z][a-z]+(\s[A-Z][a-z]+)+\s(Time|Standard Time|Daylight Time)$/,
  );
...
export function formatSmartDateTime(
  ...
  const threshold =
    options.relativeThresholdHours ?? DateTimeConfig.relativeThresholdHours;
...
export function formatProjectToLocalAbsolute(
  dateTimeStr: string,
  options?: DateTimeFormatOptions,
): string {
