...
const Streams = memo(function Streams() {
  ...
  const [activeCategory, setActiveCategory] =
    useState<GameCategory>("favorites");
  const [gameSort, setGameSort] = useState<GameSortMode>("lastPlayedNewest");
  const activeGames = useMemo(
    () => sortGames(gamesByCategory(activeCategory), gameSort),
    [activeCategory, gameSort],
  );
  ...
  const [remainingMs, setRemainingMs] = useState<number>(
    SETUP_CYCLE_INTERVAL_MS,
  );

  useEffect(() => {
    if (isPaused) return;
    cycleStartRef.current =
      Date.now() - (SETUP_CYCLE_INTERVAL_MS - remainingMs);
  ...
  const { schedule, timezoneAbbr } = useMemo(() => {
    ...
    const converted = SCHEDULE.map((item) => ({
      ...item,
      localTime: item.streamTime
        ? convertScheduleTimeToLocal(item.streamTime)
        : null,
    }));
                ...
                return (
                  ...
                  >
                    <li className="flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-gradient-to-r from-muted/15 to-transparent border border-border/20 transition-all duration-300 hover:border-border/40">
                      <div
                        className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-primary/10 border border-primary/20 shrink-0"
                        aria-hidden="true"
                      >
          ...
          <div className="max-w-6xl 3xl:max-w-7xl 4xl:max-w-[100rem] 5xl:max-w-[120rem] mx-auto">
            <SectionHeading
              ...
              subtitle={
                <>
                  Streams happen{" "}
                  <span className="text-foreground font-medium">often</span>.
                  Best way to catch them is follow and turn on notifications.
                </>
              }
            ...
            <ScrollRevealSection delay={0.15}>
              <p className="text-sm sm:text-base 3xl:text-lg text-muted-foreground/70 text-center mb-8 sm:mb-10 lg:mb-12 italic">
                Times shown in your local timezone ({timezoneAbbr}). Things may
                shift depending on life.
              </p>
            </ScrollRevealSection>
            ...
            <ScrollRevealSection delay={0.2}>
              <div
                ...
                role="list"
                aria-label="Weekly stream schedule"
              >
                {schedule.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center min-h-[50px] sm:min-h-[60px] lg:min-h-[80px]">
                      {item.localTime ? (
                        <>
                          <span className="text-sm sm:text-lg lg:text-2xl font-bold text-primary whitespace-nowrap">
                            {item.localTime}
                          </span>
                          <span className="text-[9px] sm:text-[10px] lg:text-xs text-muted-foreground mt-1">
                            {timezoneAbbr}
                          </span>
                        </>
                      ) : item.isRandom ? (
                        <div className="text-center">
                          <span className="text-xl sm:text-2xl lg:text-3xl text-muted-foreground/50">
                            ?
                          </span>
            ...
            <ScrollRevealSection delay={0.5}>
              <p className="text-center text-muted-foreground/60 mt-8 text-sm">
                <span className="text-primary">?</span> = One random weekday
                stream each week!
              </p>
            </ScrollRevealSection>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
              {WATCH_PLATFORMS.map((platform, index) => {
                ...
                const wrapperProps = isLive
                  ...
                  : {};
                const iconKey =
                  platform.name.toLowerCase() as keyof typeof SOCIAL_ICONS;
                  ...
                  >
                    <Wrapper
                      ...
                      className={`group relative h-full flex flex-col p-6 lg:p-7 rounded-2xl border overflow-hidden transition-all duration-300 ${
                        isLive ? "hover:scale-[1.01]" : "opacity-85"
                      ...
                      style={{
                        ...
                        borderColor: isLive
                          ? `${platform.gradientFrom}77`
                          : `${platform.gradientFrom}33`,
                      }}
                    >
                      <div
                      ...
                      <div className="relative flex items-center gap-4 4xl:gap-5 mb-4 4xl:mb-5">
                        ...
                        <div className="flex flex-col min-w-0">
                          <h3 className="text-xl lg:text-2xl 4xl:text-3xl 5xl:text-4xl font-bold leading-tight">
                            {platform.name}
                          </h3>
                          {isLive ? (
                      ...
                      {isLive && (
                        <div
                          className="relative flex items-center gap-1.5 text-sm font-medium mt-5 group-hover:gap-2.5 transition-all"
                          style={{ color: platform.gradientFrom }}
                        >
                          <span>Watch now</span>
      ...
      {isSectionVisible("streams.setup") && (
        <section className="py-32 px-6">
          <div className="max-w-4xl 3xl:max-w-5xl 4xl:max-w-6xl 5xl:max-w-7xl mx-auto">
            <SectionHeading
              title="Setup"
              subtitle="Curious about what powers the streams?"
              subtitleMarginClassName="mb-8 sm:mb-12"
            />
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <dd
                          className={`text-lg font-medium mt-1 h-7 relative overflow-hidden transition-opacity duration-500 ${showActual ? "opacity-100" : "opacity-70"}`}
                        >
                          <AnimatePresence mode="wait">
                            <motion.span
                              key={currentValue}
