...
export type ViewerPlatform = "twitch" | "youtube" | "tiktok" | "kick" | "trovo";
...
export interface ViewerIdentity {
  ...
  tiktok?: ViewerPlatformAccount;
  kick?: ViewerPlatformAccount;
...
const PUBLIC_SEED: Viewer[] = [
  buildViewer({
    login: "roboticlayman",
    display: "RoboticLayman",
    chips: 3_726_296,
    watchMinutes: 112_329,
    role: 4,
    subscribed: true,
  }),
  buildViewer({
  ...
  buildViewer({
    login: "letslaygamer",
    display: "letslaygamer",
    chips: 29_141_930,
    watchMinutes: 15_176,
    role: 1,
  }),
  buildViewer({
    login: "gravesforghosts",
    display: "GravesForGhosts",
    chips: 0,
    watchMinutes: 14_686,
    role: 1,
  }),
  ...
  buildViewer({
    login: "xxspookyghostboixx",
    display: "xxspookyghostboixx",
    chips: 7_227_149,
    watchMinutes: 12_458,
    role: 1,
  }),
  ...
  buildViewer({
    login: "gettinginvestigated",
    display: "gettinginvestigated",
    chips: 3_317_214,
    watchMinutes: 0,
    role: 1,
  }),
  buildViewer({
    login: "whackypancake22",
    display: "whackypancake22",
    chips: 1_740_899,
    watchMinutes: 0,
    role: 1,
  }),
...
const localModules = import.meta.glob<{ VIEWERS_LOCAL: Viewer[] }>(
  "./viewers.local.ts",
  { eager: true },
);
const localViewers: Viewer[] | null = (() => {
  for (const mod of Object.values(localModules)) {
    if (
      mod &&
      Array.isArray((mod as { VIEWERS_LOCAL?: Viewer[] }).VIEWERS_LOCAL)
    ) {
...
const excludedSet = new Set(
  LEADERBOARD_EXCLUDED_LOGINS.map((l) => l.toLowerCase()),
);
...
export function useTopViewers(metric: ViewerMetric, limit = 10): Viewer[] {
  const key: keyof ViewerEconomy =
    metric === "chips" ? "chips" : "watchMinutes";
  return VIEWERS.filter((v) => !isExcludedFromLeaderboards(v))
