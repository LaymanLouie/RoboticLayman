...
import { visibleNavRoutes, CURRENT_ROLE } from "@/config/accessConfig";

const NAV_ITEMS = visibleNavRoutes(CURRENT_ROLE);
import { useIsScrolled } from "@/contexts/LayoutContext";
import { BrandName } from "@/components/primitives/GradientText";
import avatarClear from "@/assets/avatars/avatar-clear.png";

const IDLE_GLOW_TIMEOUT = 10000;
const GLOW_STEP_DURATION = 1500;

const BRAND_GLOW =
  "0 0 12px rgba(255,255,255,0.7), 0 0 20px rgba(187,102,255,0.5), 0 0 30px rgba(136,0,255,0.3)";
const WHITE_GLOW =
    ...
    >
      <div className="w-full max-w-[95vw] mx-auto px-4">
        ...
        >
          <Link
            to="/"
            className={`group relative flex items-center justify-center transition-all duration-500 ease-out -ml-3`}
          >
            <div
              className={`absolute inset-0 rounded-xl bg-gradient-to-b from-[#8800FF] to-[#220033] transition-opacity duration-300 ${
            ...
            {!isHomePage && (
              <div
                className="absolute inset-0 rounded-xl bg-gradient-to-b from-[#8800FF]/50 to-[#220033]/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                aria-hidden="true"
              />
            )}
            ...
            <div className="relative flex items-center justify-center px-2 py-1.5">
              ...
              >
                ...
                >
                  <span
                    className="font-semibold text-lg whitespace-nowrap ml-3 mr-1 pr-2"
